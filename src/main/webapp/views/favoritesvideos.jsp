	...
	<div class="row">
							...
							<form id="like-share-form" method="get">
								...
								<c:forEach var="favorite" items="${favorites}">
									...
									<c:if test="${user1.getId() == user.getId()}">
										<a href="/asm/like/remove?id=${item.getId() }&url=hobbies"
								...
								<c:if test="<%=i%>">
									<a href="/asm/like/add?id=${item.getId() }&url=hobbies"
					...
					<nav aria-label="Page navigation">
						<ul class="pagination justify-content-center">
							<li class="page-item"><a class="page-link"
								href="/asm/hobbies?page=<%=(request.getParameter("page") == null
		? 1
		: Integer.valueOf(request.getParameter("page")) > 1
				...
				: request.getParameter("page"))%>&title=<%=(request.getParameter("title") == null ? "%%" : request.getParameter("title"))%>"
							...
							%>
							<li class="page-item"><a class="page-link"
								href="/asm/hobbies?page=<%=i + 1%>&title=<%=(request.getParameter("title") == null ? "%%" : request.getParameter("title"))%>"><%=i + 1%></a></li>
							<%
							i++;
